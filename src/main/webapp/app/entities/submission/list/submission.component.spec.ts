...
import { sampleWithRequiredData } from '../submission.test-samples';
...
describe('Submission Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.submissions?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to submissionService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getSubmissionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSubmissionIdentifier).toHaveBeenCalledWith(entity);
